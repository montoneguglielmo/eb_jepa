...
class StateOnlyPredictor(SimplePredictor):
    ...
    def forward(self, x, a, **kwargs):
        ...
        next_states = []
        B, C, T, _, _ = x.shape
        if 'past_predictions' in kwargs:
            past_predictions = kwargs['past_predictions']
            # For multiple past predictions
            for i in range(self.context_length-len(past_predictions)):
                next_states.append(x[:,:,i+1:T-self.context_length+i+1])
            
            for i in range(len(past_predictions)):
                cut = len(past_predictions) - i -1
                if cut > 0:
                    next_states.append(past_predictions[i][:,:,:-cut])
                else:
                    next_states.append(past_predictions[i])
            
            combined_xa = torch.cat(next_states, dim=1)
            combined_xa = combined_xa[:,-(C*self.context_length):]
        else:
